...
generating a quote image and idles near zero otherwise, and the whole thing (bot + venv + built front + all dependencies)
takes up roughly 220MB of disk (100MB if you drop `node-modules`). Nothing exotic here,
...
- Python 3.12+
- Node.js + npm (only if you're building the web front yourself. The CI/CD pipeline can do this for you instead, see [CI/CD](#cicd) below)
...
2. Create a venv and install the deps
   ...
   ```
   Package names can drift a bit between distro versions, but if `pip install` dies specifically on the Pillow line,
   this is almost certainly why - see [Pillow's own install docs](https://pillow.readthedocs.io/en/stable/installation/building-from-source.html) if the above doesn't cover it.
...
The front talks to the backend through same-origin `/api/...` calls, so it expects something in front of it doing the
routing rather than hitting the backend directly - see below.

Don't want Node installed locally at all? Wire up the CI/CD pipeline instead (see [CI/CD](#cicd)) - it builds
`front/dist` on every push and can ship it straight to your server.

#### Tying it together

Both `/quotes/...`, `/authors`, `/login`, etc. from the backend and the built front need to be served from the same
origin under `/` and `/api/`. An example nginx config for that is in [`deploy/nginx.conf.example`](deploy/nginx.conf.example) - point its
`root` at your `front/dist` and it'll proxy `/api/` to the bot's `LOCAL_ADDRESS:LOCAL_PORT`. Without a proxy in front of
it (nginx or otherwise), the web front can't reach the backend, `npm run dev` included.
...
Both mount points stay active on the same hostname at once, `tailscale serve` strips the `/api` prefix before forwarding
just like the nginx config does (so `/api/login` reaches the bot as `/login`), and you get HTTPS for free. Two things it
won't do for you though: the extra security headers (HSTS, `X-Frame-Options`, etc.) the example nginx config sets, and a
custom domain - by default it's reachable at `https://<device>.<tailnet>.ts.net` to your tailnet, or to the whole
internet if you turn on [Funnel](https://tailscale.com/kb/1223/funnel).
...
- **Backend** - lints with `ruff` and runs the `pytest` suite.
- **Frontend** - installs deps and runs `npm run build` and uploads `front/dist` as a build artefact.

On a push to `main` (not on forks or PRs) a third job takes that build and deploys it to "production" (the board this project actually runs on).
Connection is handled by Tailscale shipping the built front over `scp` and then running [`deploy/deploy.sh`](deploy/deploy.sh) remotely.\
The deployment script consists of the following steps:
1. Checks out the new commit(s)
2. Reinstalls Python deps only if `requirements.txt` changed
3. Swaps in the new front build
4. Restarts the service and health-checks it (If anything fails it automatically rolls back to previous version)

That deploy job is pinned to this repository and needs its own secrets (`DEPLOY_HOST`, `DEPLOY_USER`, Tailscale OAuth
credentials) to run, so you won't get it "for free" on a fork :>. The lint/test/build jobs work out of the box and if
you want the same auto-deploy for your own server, `ci.yml` and `deploy.sh` are there as a working example to adapt.

## Nice to have
- own domain (or just relying on `tailscale serve` / `tailscale funnel`, see the note in [Tying it together](#tying-it-together) it can
fully replace nginx here)
